...
const CommonPortfilo = ({ condition, items }) => {
  ...
  const categories = ["All", "UX/UI", "Code"];
  return (
    <>
      ...
      <section className="bg-white  lg:rounded-2xl dark:bg-[#111111]">
        <div data-aos="fade">
          <div className="container mb-8   px-4 sm:px-5 md:px-10 lg:px-[60px]">
            <div className="py-12">
              <h2 className="8effect  after:left-48 mt-12  lg:mt-0">
                Portfolio
              </h2>
              ...
              >
                {categories.map((category) => (
                  <li
                    key={category}
                    className={`${
                      test === category ? "text-[#FA5252]" : "fillter-btn"
                    } ${category === "UI/UX" ? "ml-0" : ""} mr-4 md:mx-4`}
                    onClick={() => handleSearch(category)}
                  >
                    {category === "UI/UX" ? "Graphic Design" : category}
                  </li>
                ))}
              </ul>
